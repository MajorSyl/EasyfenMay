...
export default function ListingDetailView({ listing }: { listing: Listing }) {
  ...
  useEffect(() => {
    ...
    // In production: trigger view count via API
  }, [listing.id]);
      ...
      <div className="flex-1 overflow-y-auto pb-28 no-scrollbar">
        ...
        <div className="px-5 py-6 max-w-2xl mx-auto w-full">
          ...
          <div className="flex items-center text-slate-500 mb-6">
            ...
            <span className="text-sm font-medium">{listing.location_name || 'Unknown Location'}</span>
          </div>
          ...
          <div className="rounded-2xl border border-slate-100 bg-white shadow-sm overflow-hidden mb-8 w-full max-w-sm">
            <div className="p-5 flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <h4 className="font-bold text-lg text-slate-900 leading-tight mb-1 truncate">
                ...
                <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">
                  <span className="bg-slate-100 px-2 py-0.5 rounded-full text-slate-500">
                    {(listing.profiles?.role || 'user').replace('_', ' ')}
                  </span>
            ...
            <div className="px-5 py-3 bg-slate-50 border-t border-slate-100 flex justify-between items-center cursor-pointer hover:bg-slate-100 transition-colors" onClick={() => { setSelectedListing(null); setCurrentView('profile'); }}>
      ...
      <div className="absolute bottom-0 w-full p-4 bg-white border-t border-slate-100 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] z-20 pb-8 flex justify-center">
        <div className="flex gap-3 w-full max-w-2xl">
          <button
            ...
            className="flex-1 bg-sky-500 hover:bg-sky-600 active:bg-sky-700 text-white shadow-xl shadow-sky-500/30 rounded-xl h-14 flex items-center justify-center font-bold text-lg transition-colors gap-2 cursor-pointer"
          >
